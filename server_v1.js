...
const postObj=req.body
  ...
  try {
    const tabsQuery = pool.query("select * from user_info.user_data;");
...
const email=postObj.email
  try {
    const stmt = 'INSERT INTO user_info.user_data (firstName,middleName,lastName,password,email) values (?,?,?,?,?);'
...
const postObj=req.body
  ...
  try {
    const tabsQuery = pool.query("select * from user_info.user_profile;");
        let x = await tabsQuery;
res.json(x);
} catch (err) {
        console.error(err);
...
const timestamp = new Date();
const postObj=req.body
const profileName=postObj.profileName
const USER_ID=postObj.USER_ID
const allergies=postObj.allergies
const foodRestrictions=postObj.foodRestrictions
const favCuisines=postObj.favCuisines
const favRecipes=postObj.favRecipes
  try {
    const stmt = 'INSERT INTO user_info.user_profile (profileName,USER_ID,allergies,foodRestrictions,favCuisines,favRecipes) values (?,?,?,?,?,?);'
    await pool.query(stmt,[profileName,USER_ID,allergies,foodRestrictions,favCuisines,favRecipes]);
  } catch (err) {
    console.error(err);
    return res
      .status(500)
...
const USER_ID=postObj.USER_ID
  try {
const stmt = 'delete from user_info.user_profile where profileName=? and USER_ID=?';
    await pool.query(stmt,[profileName,USER_ID]);
  } catch (err) {
    console.error(err);
    return res
      .status(500)
